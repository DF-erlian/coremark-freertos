...
int print_float(char **out, float i, int b, int sg, int width, int pad, int letbase, char *buflimit){
	char print_buf[PRINT_BUF_LEN + 1 + 6]; 
	register char *s;
	register int t, neg = 0, pc = 0;
	register unsigned int u = (unsigned int)i;
	register float d  = i - (int) i;
	register int cnt = 6;
	...
	s = print_buf + PRINT_BUF_LEN + 1 + 6 - 1;
	*s = '\0';
	s = s - 6;

	//print the decimal part of float
	while(cnt){
		d *= 10;
		t = (int) d;
		*s++ = (char)(t + '0');
	...
	*s = '.';
	...
	//print the integer part of float
	if (sg && b == 10 && i < 0) {
		neg = 1;
		u = (unsigned int)-i;
	}
...
static int tiny_print( char **out, const char *format, va_list args, unsigned int buflen )
{
	...
	for (; *format != 0; ++format) {
		if (*format == '%') {
			...
			if( *format == 'f'){
				pc += print_float (out, va_arg( args, double ), 10, 1, width, pad, 'a', buflimit);
				continue;
			}
		}
		else {
...
int printf_main(void)
{
	...
	printf("CoreMark 1.0 : %f / %s %s \n",
                      43544.524276,
                      "/ GCC11.4.0 -O2 -DPERFORMANCE_RUN=1  -lrt /",
                      "Heap");
